...
K &=& \mathrm{diag}(\bvec{a}) + \mathrm{tril}(\tilde{U}\,\tilde{V}^\T) +
    \mathrm{triu}(\tilde{V}\,\tilde{U}^\T)
\end{eqnarray}
where $\bvec{a}$ is a $N$-vector, $\tilde{U}$ and $\tilde{V}$ are $N \times J$
...
\begin{eqnarray}
\bvec{u}_n &\gets&
\left[\prod_{k=1}^{n-1}\mathrm{diag}(\bvec{p}_k)^{-1}\right]\,\bvec{u}_n\\
\bvec{v}_n &\gets&
\left[\prod_{k=1}^{n-1}\mathrm{diag}(\bvec{p}_k)\right]\,\bvec{v}_n\\
\bvec{w}_n &\gets&
\left[\prod_{k=1}^{n-1}\mathrm{diag}(\bvec{p}_k)\right]\,\bvec{w}_n\\
\end{eqnarray}
and in all that follows, I will use these transformed
variables\footnote{\citet{Foreman-Mackey:2017} indicate the transformed
variables using a tilde, but that notation will be inconsistent later.}.
